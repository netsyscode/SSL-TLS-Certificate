...
class IPScanner(Scanner):
    ...
    def scan_thread(self, ip : str):
        ...
        '''
        cert_chain, e = self.fetch_raw_cert_chain(ip, None, proxy_host=None, proxy_port=None)

        # print(len(cert_chain), e)
        if len(cert_chain) == 0:
            # my_logger.warning(f"{host} using VPN proxy data...")
            cert_chain, e = self.fetch_raw_cert_chain(ip, None, proxy_host=self.proxy_host, proxy_port=self.proxy_port)
        ...
        '''
        result = {'ip': ip, 'error': e, 'certificate': cert_chain, 'sha256' : cert_chain_sha256_hex}
