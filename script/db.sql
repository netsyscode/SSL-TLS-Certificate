...
CREATE TABLE IF NOT EXISTS `CERT_REVOCATION_STATUS_CRL` (
  ...
  `REVOCATION_STATUS` INT NULL DEFAULT '0',
  `REVOCATION_TIME` DATETIME NULL DEFAULT NULL,
  `REVOCATION_REASON` INT NULL DEFAULT NULL,
  PRIMARY KEY (`CERT_ID`, `CHECK_TIME`) USING BTREE,
...
-- ca_crypto_store 存储所有CA的CA证书以及签发用的所有密钥
CREATE TABLE IF NOT EXISTS `CA_CRYPTO_STORE` (
  `CA_ID` INT NOT NULL AUTO_INCREMENT,
  `CA_COMMON_NAME` VARCHAR(128) CHARACTER SET gbk COLLATE gbk_chinese_ci NULL,
  `CA_ORG_NAME` VARCHAR(128) CHARACTER SET gbk COLLATE gbk_chinese_ci NULL,
  `CA_COUNTRY_NAME` VARCHAR(16) CHARACTER SET gbk COLLATE gbk_chinese_ci NULL,
  `CA_ISSUING_CERTS` JSON NULL COMMENT 'use sha256',
  `CA_ISSUING_KEYS` JSON NULL COMMENT 'store both key ID and PEM value',
  PRIMARY KEY (`CA_ID`) USING BTREE,
  INDEX `index` (`CA_COMMON_NAME` ASC, `CA_ORG_NAME` ASC, `CA_COUNTRY_NAME` ASC) VISIBLE
) ENGINE = InnoDB DEFAULT CHARACTER SET = gbk COLLATE = gbk_chinese_ci ROW_FORMAT=DYNAMIC;
