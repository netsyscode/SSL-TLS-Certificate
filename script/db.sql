...
CREATE TABLE IF NOT EXISTS `CERT_STORE_CONTENT` (
  ...
  `SUBJECT_CN` varchar(512) CHARACTER SET gbk COLLATE utf8mb4_unicode_ci,
  `ISSUER_CN` VARCHAR(128) CHARACTER SET gbk COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL,
  `ISSUER_ORG` varchar(128) CHARACTER SET gbk COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL,
  `ISSUER_COUNTRY` VARCHAR(16) CHARACTER SET gbk COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `CA_CERT_STORE` (
  ...
  `CA_COMMON_NAME` VARCHAR(128) CHARACTER SET gbk COLLATE utf8mb4_unicode_ci NULL,
  `CA_ORG_NAME` VARCHAR(128) CHARACTER SET gbk COLLATE utf8mb4_unicode_ci NULL,
  `CA_COUNTRY_NAME` VARCHAR(16) CHARACTER SET gbk COLLATE utf8mb4_unicode_ci NULL,
  PRIMARY KEY (`CERT_ID`) USING BTREE,
...
CREATE TABLE IF NOT EXISTS `CERT_REVOCATION_STATUS_OCSP` (
  ...
  `AIA_LOCATION` TEXT CHARACTER SET gbk COLLATE gbk_chinese_ci NOT NULL,
  `ISSUER_ID` varchar(64) CHARACTER SET gbk COLLATE gbk_chinese_ci NOT NULL,
  `REVOCATION_STATUS` INT DEFAULT NULL,
  `REVOCATION_TIME` DATETIME NULL DEFAULT NULL,
  `REVOCATION_REASON` INT NULL DEFAULT NULL,
...
-- cert_revocation_status_crl 存储使用CRL验证证书吊销状态的信息
CREATE TABLE IF NOT EXISTS `CERT_REVOCATION_STATUS_CRL` (
  `CERT_ID` VARCHAR(64) CHARACTER SET gbk COLLATE gbk_chinese_ci NOT NULL,
  `CHECK_TIME` DATETIME NOT NULL,
  `CRL_POSITION` TEXT CHARACTER SET gbk COLLATE gbk_chinese_ci NOT NULL,
  `REVOCATION_STATUS` INT NULL DEFAULT NULL,
  `REVOCATION_TIME` DATETIME NULL DEFAULT NULL,
  `REVOCATION_REASON` INT NULL DEFAULT NULL,
  PRIMARY KEY (`CERT_ID`, `CHECK_TIME`) USING BTREE,
  KEY `FK_cert_id_cert_revocation_status_crl` (`CERT_ID`) USING BTREE,
  CONSTRAINT `FK_cert_id_cert_revocation_status_crl` FOREIGN KEY (`CERT_ID`) REFERENCES `CERT_STORE_RAW` (`CERT_ID`) ON DELETE RESTRICT
) ENGINE = InnoDB DEFAULT CHARACTER SET = gbk COLLATE = gbk_chinese_ci ROW_FORMAT=DYNAMIC;

-- crl_archive 存储CRL文件
CREATE TABLE IF NOT EXISTS `CRL_ARCHIVE` (
  `CRL_POSITION` TEXT CHARACTER SET gbk COLLATE gbk_chinese_ci NOT NULL,
  `STORE_TIME` DATETIME NOT NULL,
  `FINGERPRINT` VARCHAR(64) CHARACTER SET gbk COLLATE gbk_chinese_ci NOT NULL,
